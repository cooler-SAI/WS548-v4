...
enum Spells
{
	SPELL_EARTHQUAKE = 70670,
	SPELL_SUNDER_ARMOR = 144467,
	SPELL_CHAIN_LIGHTNING = 144483,
	SPELL_OVERRUN = 144485,
	SPELL_ENRAGE = 27650,
	SPELL_MARK_DEATH = 144492,
	SPELL_AURA_DEATH = 40496
};
...
public:
	...
	struct boss_iron_juggernautAI : public ScriptedAI
	{
		...
		{
			if (killer) { if (killer->GetTypeId() == TYPEID_PLAYER)         if (AchievementEntry const* achievementEntry = sAchievementStore.LookupEntry(8469)) killer->ToPlayer()->CompletedAchievement(achievementEntry); }Talk(SAY_DEATH);
		}
