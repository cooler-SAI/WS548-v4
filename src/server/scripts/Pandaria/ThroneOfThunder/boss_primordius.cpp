...
enum Spells
{
    SPELL_EARTHQUAKE        = 136037,
    SPELL_SUNDER_ARMOR      = 136210,
    SPELL_CHAIN_LIGHTNING   = 136215,
    SPELL_OVERRUN           = 136216,
    SPELL_ENRAGE            = 136209,
    SPELL_MARK_DEATH        = 136218,
    SPELL_AURA_DEATH        = 136228
};
...
class boss_primordius : public CreatureScript
{
    public:
        ...
        struct boss_primordiusAI : public ScriptedAI
        {
            ...
            {
                if (killer) { if (killer->GetTypeId() == TYPEID_PLAYER)         if (AchievementEntry const* achievementEntry = sAchievementStore.LookupEntry(8063)) killer->ToPlayer()->CompletedAchievement(achievementEntry); }Talk(SAY_DEATH);
            }
                ...
                while (uint32 eventId = _events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_CHAIN:
                            ...
                            break;
                        case EVENT_ARMOR:
                            Talk(irand(5,12));if (irand(0,5)==0) DoCastVictim(136246); else if (irand(0,5)==0) DoCastVictim(SPELL_MARK_DEATH);else if (irand(0,5)==1) DoCastVictim(SPELL_AURA_DEATH);else DoCastVictim(SPELL_SUNDER_ARMOR);
