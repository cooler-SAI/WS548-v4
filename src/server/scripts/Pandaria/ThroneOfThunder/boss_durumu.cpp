...
enum Spells
{
    SPELL_EARTHQUAKE        = 133765,
    SPELL_SUNDER_ARMOR      = 133767,
    SPELL_CHAIN_LIGHTNING   = 133768,
    SPELL_OVERRUN           = 134029,
    SPELL_ENRAGE            = 27650,
    SPELL_MARK_DEATH        = 41238,
    SPELL_AURA_DEATH        = 134626
};
...
class boss_durumu : public CreatureScript
{
    public:
        ...
        struct boss_durumuAI : public ScriptedAI
        {
            ...
            {
               if (killer) { if (killer->GetTypeId() == TYPEID_PLAYER)         if (AchievementEntry const* achievementEntry = sAchievementStore.LookupEntry(8062)) killer->ToPlayer()->CompletedAchievement(achievementEntry); }Talk(SAY_DEATH); 
            }
