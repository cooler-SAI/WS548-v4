...
enum Spells
{
    SPELL_EARTHQUAKE        = 134366,
    SPELL_SUNDER_ARMOR      = 134370,
    SPELL_CHAIN_LIGHTNING   = 134380,
    SPELL_OVERRUN           = 138052,
    SPELL_ENRAGE            = 27650,
    SPELL_MARK_DEATH        = 138923,
    SPELL_AURA_DEATH        = 140092
};
...
class boss_jikun : public CreatureScript
{
    public:
        ...
        struct boss_jikunAI : public ScriptedAI
        {
            ...
            {
                if (killer) { if (killer->GetTypeId() == TYPEID_PLAYER)         if (AchievementEntry const* achievementEntry = sAchievementStore.LookupEntry(8061)) killer->ToPlayer()->CompletedAchievement(achievementEntry); }Talk(SAY_DEATH);
            }
