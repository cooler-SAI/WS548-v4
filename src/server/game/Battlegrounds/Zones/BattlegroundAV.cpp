...
{
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "ALterac Valley: HandleKillUnit %i", unit->GetEntry());
        ...
        case AV_QUEST_H_SCRAPS2:
            ...
            if (m_Team_QuestStatus[team][0] == 500 || m_Team_QuestStatus[team][0] == 1000 || m_Team_QuestStatus[team][0] == 1500) // 25, 50, 75 turn-ins.
            {
                ...
                for (BG_AV_Nodes i = BG_AV_NODES_FIRSTAID_STATION; i <= BG_AV_NODES_FROSTWOLF_HUT; ++i)
                    ...
                    {
                        DePopulateNode(i);
                        PopulateNode(i); // maybe this is bad, because it will instantly respawn all creatures on every grave..
                    }
            }
            break;
...
Creature* BattlegroundAV::AddAVCreature(uint16 cinfoid, uint16 type)
{
    ...
    if ((isStatic && cinfoid >= 10 && cinfoid <= 14) || (!isStatic && ((cinfoid >= AV_NPC_A_GRAVEDEFENSE0 && cinfoid <= AV_NPC_A_GRAVEDEFENSE3) || (cinfoid >= AV_NPC_H_GRAVEDEFENSE0 && cinfoid <= AV_NPC_H_GRAVEDEFENSE3))))
    {
        ...
        //creature->SetDefaultMovementType(RANDOM_MOTION_TYPE); // TODO: Add a motionmaster without killing the creature.
...
void BattlegroundAV::EndBattleground(uint32 winner)
{
    ...
    for (int i = 0; i < 2; i++) // 0 = ally 1 = horde
    {
        if (m_CaptainAlive[i])
        {
...
// Changing the owner results in setting respawntime to infinite for current creatures, spawning new mine owners creatures and 
// changing the chest-objects so that the current owning team can use them.
void BattlegroundAV::ChangeMineOwner(uint8 mine, uint32 team, bool initial)
{
    ...
    // Also a neutral team exists.. after a big time, the neutral team tries to conquer the mine.
    uint16 miner;
...
void BattlegroundAV::PopulateNode(BG_AV_Nodes node)
{
    ...
    // Cast bonus aura (+50% honor in 25yards), ura should only apply to players who have accupied the node, set correct faction for trigger.
    if (trigger)
    {
...
void BattlegroundAV::EventPlayerDefendsPoint(Player* player, uint32 object)
{
    ...
    // Until snowfall belongs to anyone it is better handled in assault-code.
    ...
    sLog->outDebug(LOG_FILTER_BATTLEGROUND, "A player defends a point object: %i node: %i", object, node);

    if (m_Nodes[node].PrevOwner != team)
    {
        sLog->outError(LOG_FILTER_BATTLEGROUND, "BG_AV: A player defends a point which doesn't belong to his team %i", node);
        return;
    }
    ...
    // Update the statistics for the defending player.
...
void BattlegroundAV::EventPlayerAssaultsPoint(Player* player, uint32 object)
{
    ...
    // If snowfall got capped it can be handled like all other graveyards.
    if (m_Nodes[node].TotalOwner != AV_NEUTRAL_TEAM)
    {
        ...
        else
        {
            ...
            SpawnBGObject(BG_AV_OBJECT_AURA_N_FIRSTAID_STATION+3*node, RESPAWN_IMMEDIATELY); // Neutral aura spawn.
            SpawnBGObject(BG_AV_OBJECT_AURA_A_FIRSTAID_STATION+GetTeamIndexByTeamId(owner)+3*node, RESPAWN_ONE_DAY); // Team aura despawn.
    ...
    SpawnBGObject(object, RESPAWN_ONE_DAY); // Delete the old banner.
...
void BattlegroundAV::FillInitialWorldStates(ByteBuffer &data)
{
    ...
    {
        for (uint8 j = 1; j <= 3; j += 2) // j = 1 = assaulted j = 3 = controled // i dont have j = 2 = destroyed cause destroyed is the same like enemy-team control.
        {
    ...
    // Only if game started are the teamscores displayed.
    if (GetStatus() == STATUS_IN_PROGRESS)
    { 
...
uint8 BattlegroundAV::GetWorldStateType(uint8 state, uint16 team)
{
...
void BattlegroundAV::UpdateNodeWorldState(BG_AV_Nodes node)
{
    ...
    if (m_Nodes[node].PrevOwner == AV_NEUTRAL_TEAM)
...
void BattlegroundAV::SendMineWorldStates(uint32 mine)
{
    ...
    uint8 owner, prevowner, mine2; // These variables are needed to access the right worldstate in the BG_AV_MineWorldStates array.
...
bool BattlegroundAV::SetupBattleground()
{
    ...
    // Spawn gates.
    if (!AddObject(BG_AV_OBJECT_DOOR_A, BG_AV_OBJECTID_GATE_A, BG_AV_DoorPositons[0][0], BG_AV_DoorPositons[0][1], BG_AV_DoorPositons[0][2], BG_AV_DoorPositons[0][3], 0, 0, std::sin(BG_AV_DoorPositons[0][3] / 2), std::cos(BG_AV_DoorPositons[0][3] / 2), RESPAWN_IMMEDIATELY)
    ...
    for (uint8 i = BG_AV_NODES_FIRSTAID_STATION; i < BG_AV_NODES_MAX; ++i)
    {
        ...
        else // Towers.
        {
            ...
            for (uint8 j = 0; j <= 9; j++) // Burning auras.
            {
    ...
    for (uint8 i = 0; i < 2; i++) // Burning auras for buildings.
    {
        for (uint8 j = 0; j <= 9; j++)
        {
            if (j < 5)
...
const char* BattlegroundAV::GetNodeName(BG_AV_Nodes node)
{
    ...
    return "BG_AV: Unknown Node Name!";
}
...
void BattlegroundAV::ResetBGSubclass()
{
    ...
    for (uint8 i = 0; i < 2; i++) // For loop for both teams (It just make 0 == alliance and 1 == horde also for both mines 0 = north 1 = south).
    {
        for (uint8 j = 0; j < 9; j++)
        ...
        m_CaptainBuffTimer[i] = 120000 + urand(0, 4) * 60; // As far as i could see, the buff is randomly so i make 2 minutes (thats the duration of the buff itself) + 0 - 4 minutes TODO get the right times.
