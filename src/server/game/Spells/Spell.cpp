...
void Spell::SendChannelUpdate(uint32 time)
{
    ...
    WorldPacket data(SMSG_SPELL_CHANNEL_UPDATE, 1 + 8 + 4);
...
void Spell::SendChannelStart(uint32 duration)
{
    ...
    WorldPacket data(SMSG_SPELL_CHANNEL_START);
    ...
    if (HasHealPrediction)
    {
        ...
        data.WriteBit(!HasTypePrediction);
    ...
    if (HasImmunity)
        data << uint32(0) << uint32(0); // ImmunitySchools, Immunity, if it doesn't work, swap them.
...
SpellCastResult Spell::CheckCast(bool strict)
{
    ...
    for (uint8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
    {
        switch (m_spellInfo->Effects[i].ApplyAuraName)
        {
            ...
            {
                // Except for water mounts, this fixes Vashj'ir and older ones.
                if (m_caster->IsInWater() && !(m_spellInfo->AttributesEx8 & SPELL_ATTR8_WATER_MOUNT))
...
void Spell::CallScriptBeforeCastHandlers()
{
    ...
    if (scriptExecuteTime > 15)
        sLog->OutSpecialLog("SpellScript [%u] take more than 15 ms to execute (%u ms)", m_spellInfo->Id, scriptExecuteTime);
}
