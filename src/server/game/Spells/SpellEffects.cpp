      ...
      if (unitTarget && unitTarget->isAlive())
      {
            ...
            case SPELLFAMILY_MONK:
            {
                                       ...
                                       case 100787:// Tiger Palm
                                             ...
                                             if (m_caster->HasAura(145004) && m_caster->HasAura(118864))
                                                   ApplyPct(damage, 40);
                                             break;
...
void Spell::EffectEnergize(SpellEffIndex effIndex)
{
      ...
      if (power == POWER_RAGE)
      {
            if (m_spellInfo->Id == 6572 || m_spellInfo->Id == 23922)
            if (m_caster->HasAura(138281) && m_caster->HasAura(125565))
                  AddPct(damage, 50);
      }
      else if (power == POWER_CHI)
      {
            if (roll_chance_i(15))
            {
                  m_caster->CastSpell(m_caster, 138310, true);
            }
      }
...
void Spell::EffectSummonType(SpellEffIndex effIndex)
{
      ...
      case SUMMON_CATEGORY_UNK:
            ...
            case SUMMON_TYPE_TOTEM:
            {
                                        ...
                                        {
                                              summon->SetMaxHealth(damage != 10 /*Mana Tide Totem - 16190*/ ? (m_caster->HasAura(63298) ? m_caster->CountPctFromMaxHealth(5) : damage) : m_caster->CountPctFromMaxHealth(m_caster->HasAura(63298) ? 15 : 10));
                                              summon->SetHealth(damage != 10 /*Mana Tide Totem - 16190*/ ? (m_caster->HasAura(63298) ? m_caster->CountPctFromMaxHealth(5) : damage) : m_caster->CountPctFromMaxHealth(m_caster->HasAura(63298) ? 15 : 10));
                                        }
                                        break;
            }
...
void Spell::EffectInterruptCast(SpellEffIndex effIndex)
{
      ...
      for (uint32 i = CURRENT_FIRST_NON_MELEE_SPELL; i < CURRENT_AUTOREPEAT_SPELL; ++i)
      {
            if (Spell* spell = unitTarget->GetCurrentSpell(CurrentSpellTypes(i)))
            {
                  ...
                  if (curSpellInfo->Id == 127663) // Astral Communion is now immune to interrupts.
                        return;
